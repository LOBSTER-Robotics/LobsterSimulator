...
class Lobster:
    ...
    def __init__(self, length, diameter, arm_length, arm_position_from_center, center_of_mass,
                 inner_motor_distance_from_center, max_rpm_change_per_second):
    ...
    def update(self, dt):
        ...
        for i in range(4):
            p.applyExternalForce(objectUniqueId=self.id, linkIndex=-1,
                                 forceObj=[0, 0, rpm_to_thrust(self.rpm_motors[i])], posObj=self.motorPositions[i],
                                 flags=p.LINK_FRAME)
