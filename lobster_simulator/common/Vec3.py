...
class Vec3:
    ...
    def __init__(self, data: Union[List[float], np.ndarray]):
        ...
        if not (isinstance(data, np.ndarray) or isinstance(data, List) or isinstance(data, Tuple)):
            raise InvalidArgumentTypeError(f"A Vec3 needs to be instantiated by a list of floats or a numpy array not a {type(data)}")
        ...
        if self._data.shape[0] != 3:
            ...
            raise InvalidArgumentTypeError(f"A Vec3 needs to be instantiated by an array of floats, not an array of {self._data.dtype}")
