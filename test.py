...
def get_height(x, y):
    # math.sin(x / 100) * 10 +
    return math.sin(y / 100)

def load_chunk(x, y):
    numHeightfieldRows = 2 ** 8
    numHeightfieldColumns = 2 ** 8

    chunk_x = x * numHeightfieldRows
    chunk_y = y * numHeightfieldColumns
    ...
    for j in range(numHeightfieldColumns):
        for i in range(numHeightfieldRows):
            ...
            height = get_height(chunk_x + i, chunk_y + j)
            # height =
            # height *=100
            # print(chunk_x + i, chunk_y + j, height)
            heightfieldData[i + j*numHeightfieldRows] = height

    middle = heightfieldData[len(heightfieldData)//2 - 2**7]
    print(middle)
    # middle = numpy.mean(heightfieldData)
    ...
    result = p.rayTest([0.05 * (2**8-1) * x, 0.05 * (2**8-1) * y, -1],
                       [0.05 * (2**8-1) * x, 0.05 * (2**8-1) * y, 9])

    print(f"{result[0][2]*10 + 1:0.6f}")
